...
class upnp_device(object):
    ...
    def local_ip_address(logger=None):
        if not upnp_device.this_host_ip:
            ...
            del(temp_socket)
            if logger:
                logger.info("got local address of %s" %
    ...
    def __init__(self, listener, poller, port, root_url, server_version,
        ...
        if ip_address:
            ...
            self.ip_address = upnp_device.local_ip_address(logger)
    ...
    def respond_to_search(self, destination, search_target):
        ...
        message = ("HTTP/1.1 200 OK\r\n"
                   ...
                   "ST: %s\r\n"
                   "USN: uuid:%s::%s\r\n" % (
                    date_str, location_url, self.uuid, self.server_version,
                    search_target, self.persistent_uuid, search_target))
